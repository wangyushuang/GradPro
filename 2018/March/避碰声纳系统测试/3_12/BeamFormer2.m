...
% 先取1s的信号，检测直达波起始位置
 ...
 Tmsec=14; %截取时间长度：13ms 对应最远距离为10m
...
%  title('截取片段')
 ...
 % Hilbert变换
 % 输入：yout2 输出：yc
 pdelta=round(2*1/c*fs);%1米对应波形点数
 yc=zeros(pdelta,12);
 t=linspace(0,1e-3,p1ms);
 sRef=sin(2*pi*(90e3*t+0.5e7*t.^2));%参考信号
 theta=[-40:10:40]*pi/180+pi/2;
 phy=[-10:10:10]*pi/180;
 P=zeros(length(theta),length(phy),9);
 Pmax=0;
 for h=1:9
     for ch=1:12
            yc(:,ch)=hilbert(yout2(1+h*pdelta:(h+1)*pdelta,ch));%从两米处开始，每隔1米处理一次
     end
     R=yc.'*conj(yc);
...
%  xlabel('距离/米')
 figure
 Pmin=min(min(min(P(:,:,5:9))));
 Pmax=max(max(max(P(:,:,5:9))));
 for h=1:9
     subplot(3,3,h)
     imagesc(theta*180/pi-90,phy*180/pi,P(:,:,h)',[Pmin Pmax])
%      imagesc(theta*180/pi-90,phy*180/pi,P(:,:,h)') %各自归一化
     xlabel('方位角')
     ylabel('俯仰角')
     title([num2str(h) '-' num2str(h+1) '米'])
 end
