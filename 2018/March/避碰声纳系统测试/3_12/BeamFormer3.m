...
% 先取1s的信号，检测直达波起始位置
 ...
 theta=[-40:10:40]*pi/180;
 phy=[-10:10:10]*pi/180;
 P=zeros(length(theta),length(phy));
 Pmax=0;
 for th=1:length(theta)
     for ph=1:length(phy)
         r=xEle*cos(phy(ph))*cos(theta(th)+pi/2)+zEle*sin(phy(ph));%波程差  
         w=exp(-1j*2*pi*f*r/c);
%          P1(th,ph)=w'*R*w;
         ybm=w'*yc.';% 波束输出
         ymf=MatchedFilter_FFT(real(ybm),sRef);%波束输出的实部做匹配滤波
         P(th,ph)=abs(ymf*ymf');
         %记录能量最大点处信息
         if(P(th,ph)>Pmax)
                Pmax=P(th,ph);
 ...
 title('能量分布')
 %能量最大处的匹配滤波器输出
 ymax=MatchedFilter_FFT(real(wmax'*yc.'),sRef);
 figure
 dist=[1:length(ymax)]/fs*c/2/2-1.5; %距离
 plot(dist,ymax)
 xlabel('距离/米')
 title(['θ=' num2str(theta(th0)*180/pi) '°   φ=' num2str(phy(ph0)*180/pi) '°匹配滤波器输出波形'])
